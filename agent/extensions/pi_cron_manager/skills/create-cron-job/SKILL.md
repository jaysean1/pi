...
Create tasks for Pi Cron Manager. Every agent stage must run through an isolated `pi -p` child with extension, skill, and prompt-template discovery disabled. Task prompts must therefore be self-contained and use built-in providers and tools.
...
- Treat `--confirm-side-effects`, `--confirm-status-change`, and `--confirm-schedule-change` as records of explicit user approval, never as defaults.
